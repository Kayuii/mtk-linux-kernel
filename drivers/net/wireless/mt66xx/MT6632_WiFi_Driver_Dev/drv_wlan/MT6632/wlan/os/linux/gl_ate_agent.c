...
int SetATEDa(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv SetDa\n");
	/* xx:xx:xx:xx:xx:xx */
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
	if (rv == 6) {
		DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv SetATEDa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n",
...
int SetATESa(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv SetSa\n");
	/* xx:xx:xx:xx:xx:xx */
	rv = sscanf(prInBuf, "%x:%x:%x:%x:%x:%x", &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
	if (rv == 6) {
		DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv SetATESa Sa:%02x:%02x:%02x:%02x:%02x:%02x\n",
		       addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
...
int Set_TxBfProfileTag_InValid(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_InValid\n");

	rv = kstrtoint(prInBuf, 0, &ucInValid);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_InValid prInBuf = %s, ucInValid = %d\n", prInBuf,
		       ucInValid);
...
int Set_TxBfProfileTag_PfmuIdx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_PfmuIdx\n");

	rv = kstrtoint(prInBuf, 0, &ucProfileIdx);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_PfmuIdx prInBuf = %s, ucProfileIdx = %d\n", prInBuf,
		       ucProfileIdx);
...
int Set_TxBfProfileTag_BfType(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_BfType\n");

	rv = kstrtoint(prInBuf, 0, &ucBFType);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_BfType prInBuf = %s, ucBFType = %d\n",
			prInBuf, ucBFType);
...
int Set_TxBfProfileTag_DBW(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DBW\n");

	rv = kstrtoint(prInBuf, 0, &ucBW);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DBW prInBuf = %s, ucBW = %d\n", prInBuf, ucBW);
...
int Set_TxBfProfileTag_SuMu(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_SuMu\n");

	rv = kstrtoint(prInBuf, 0, &ucSuMu);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_SuMu prInBuf = %s, ucSuMu = %d\n", prInBuf, ucSuMu);
...
int Set_TxBfProfileTag_Mem(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_Mem\n");
...
int Set_TxBfProfileTag_Matrix(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_Matrix\n");
	...
	if (rv == 6) {
		DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv Set_TxBfProfileTag_Matrix aucInput:%d:%d:%d:%d:%d:%d\n",
		       aucInput[0], aucInput[1], aucInput[2], aucInput[3], aucInput[4], aucInput[5]);
...
int Set_TxBfProfileTag_SNR(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_SNR\n");

	rv = sscanf(prInBuf, "%d:%d:%d:%d", &aucInput[0], &aucInput[1], &aucInput[2], &aucInput[3]);
	if (rv == 4) {
		DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv Set_TxBfProfileTag_SNR aucInput:%d:%d:%d:%d\n",
...
int Set_TxBfProfileTag_SmartAnt(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_SmartAnt\n");

	rv = kstrtoint(prInBuf, 0, &ucSmartAnt);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_SmartAnt prInBuf = %s, ucSmartAnt = %d\n", prInBuf,
		       ucSmartAnt);
...
int Set_TxBfProfileTag_SeIdx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_SeIdx\n");

	rv = kstrtoint(prInBuf, 0, &ucSeIdx);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 TxBfProfileTag_SeIdx prInBuf = %s, ucSeIdx = %d\n", prInBuf, ucSeIdx);
...
int Set_TxBfProfileTag_RmsdThrd(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_RmsdThrd\n");

	rv = kstrtoint(prInBuf, 0, &ucRmsdThrd);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_RmsdThrd prInBuf = %s, ucRmsdThrd = %d\n", prInBuf,
		       ucRmsdThrd);
...
int Set_TxBfProfileTag_McsThrd(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_McsThrd\n");

	rv = sscanf(prInBuf, "%d:%d:%d:%d:%d:%d",
		    &aucInput[0], &aucInput[1], &aucInput[2], &aucInput[3], &aucInput[4], &aucInput[5]);
	if (rv == 6) {
		DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv Set_TxBfProfileTag_McsThrd aucInput:%d:%d:%d:%d:%d:%d\n",
		       aucInput[0], aucInput[1], aucInput[2], aucInput[3], aucInput[4], aucInput[5]);
...
int Set_TxBfProfileTag_TimeOut(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_TimeOut\n");

	rv = kstrtouint(prInBuf, 0, &ucTimeOut);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_TimeOut prInBuf = %s, ucTimeOut = %d\n", prInBuf,
		       ucTimeOut);
...
int Set_TxBfProfileTag_DesiredBW(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DesiredBW\n");

	rv = kstrtoint(prInBuf, 0, &ucDesiredBW);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DesiredBW prInBuf = %s, ucDesiredBW = %d\n", prInBuf,
		       ucDesiredBW);
...
int Set_TxBfProfileTag_DesiredNc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DesiredNc\n");

	rv = kstrtoint(prInBuf, 0, &ucDesiredNc);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DesiredNc prInBuf = %s, ucDesiredNc = %d\n", prInBuf,
		       ucDesiredNc);
...
int Set_TxBfProfileTag_DesiredNr(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DesiredNr\n");

	rv = kstrtoint(prInBuf, 0, &ucDesiredNr);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTag_DesiredNr prInBuf = %s, ucDesiredNr = %d\n", prInBuf,
		       ucDesiredNr);
...
int Set_TxBfProfileTagWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTagWrite\n");

	rv = kstrtoint(prInBuf, 0, &profileIdx);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTagWrite prInBuf = %s, profileIdx = %d\n", prInBuf,
		       profileIdx);
...
int Set_TxBfProfileTagRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileTagRead\n");
...
int Set_TxBfProfileDataRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfileDataRead\n");
...
int Set_TxBfProfileDataWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 TxBfProfileDataWrite\n");
...
int Set_TxBfProfilePnRead(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfilePnRead\n");

	rv = kstrtoint(prInBuf, 0, &profileIdx);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_TxBfProfilePnRead prInBuf = %s, profileIdx = %d\n",
			prInBuf, profileIdx);
...
int Set_TxBfProfilePnWrite(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 TxBfProfilePnWrite\n");
...
int Set_Trigger_Sounding_Proc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_Trigger_Sounding_Proc\n");
...
int Set_Stop_Sounding_Proc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_Stop_Sounding_Proc\n");
...
int Set_TxBfTxApply(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 TxBfTxApply\n");
...
int Set_TxBfManualAssoc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 TxBfManualAssoc\n");
...
int Set_TxBfPfmuMemAlloc(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 TxBfPfmuMemAlloc\n");

	rv = sscanf(prInBuf, "%d:%d", &ucSuMuMode, &ucWlanIdx);
	if (rv == 2) {
		DBGLOG(INIT, ERROR, "MT6632 TxBfPfmuMemAlloc ucSuMuMode = %d, ucWlanIdx = %d", ucSuMuMode, ucWlanIdx);
...
int Set_TxBfPfmuMemRelease(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 TxBfPfmuMemRelease\n");

	rv = kstrtoint(prInBuf, 0, &ucWlanId);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 TxBfPfmuMemRelease ucWlanId = %d", ucWlanId);
...
int Set_DevInfoUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 DevInfoUpdate\n");
...
int Set_BssInfoUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 BssInfoUpdate\n");
...
int Set_StaRecCmmUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_StaRecCmmUpdate\n");
...
int Set_StaRecBfUpdate(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_StaRecBfUpdate\n");
	...
	if (rv == 23) {
		/* DBGLOG(INIT, ERROR, "MT6632 Set_StaRecBfUpdate prInBuf = %s, u4WlanId = %x, u4BssId = %x, u4Aid = %x,
...
int Set_MUGetInitMCS(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUGetInitMCS\n");
	...
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Test\n");
		DBGLOG(INIT, ERROR, "MT6632 Set_MUGetInitMCS prInBuf = %s, u4groupIdx = %x", prInBuf, u4groupIdx);
...
int Set_MUCalInitMCS(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUCalInitMCS\n");
...
int Set_MUCalLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUCalLQ\n");
...
int Set_MUGetLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUGetLQ\n");
	...
	/* { */
	/* DBGLOG(INIT, ERROR, "MT6632 Set_MUGetLQ prInBuf = %s, u4Type = %x", prInBuf, u4Type); */
...
int Set_MUSetSNROffset(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetSNROffset\n");
	...
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_MUSetSNROffset prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetZeroNss(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetZeroNss\n");
	...
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_MUSetZeroNss prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetSpeedUpLQ(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetSpeedUpLQ\n");
	...
	rv = kstrtouint(prInBuf, 0, &u4Val);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_MUSetSpeedUpLQ prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
/*	UINT_32 i;
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetMUTable\n");
	...
	/* { */
	/* DBGLOG(INIT, ERROR, "MT6632 Set_MUSetMUTable prInBuf = %s, */
...
int Set_MUSetGroup(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetGroup\n");
...
int Set_MUGetQD(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUGetQD\n");
	...
	if (sscanf(prInBuf, "%x:%x", &u4SubcarrierIndex, &u4Length) == 2) {
		DBGLOG(INIT, ERROR, "MT6632 Set_MUGetQD prInBuf = %s, u4SubcarrierIndex = %x, u4Length = %x", prInBuf,
...
int Set_MUSetEnable(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetEnable\n");
	...
	rv = kstrtouint(prInBuf, 0, &u4Val);
	if (rv == 0) {
		DBGLOG(INIT, ERROR, "MT6632 Set_MUSetEnable prInBuf = %s, u4Val = %x", prInBuf, u4Val);
...
int Set_MUSetGID_UP(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUSetGID_UP\n");
...
int Set_MUTriggerTx(struct net_device *prNetDev, UINT_8 *prInBuf)
{
	...
	DBGLOG(INIT, ERROR, "MT6632 Set_MUTriggerTx\n");
...
int AteCmdSetHandle(struct net_device *prNetDev, UINT_8 *prInBuf, UINT_32 u4InBufLen)
{
	...
	while ((this_char = strsep((char **)&prInBuf, ",")) != NULL) {
		if (!*this_char)
			continue;
		DBGLOG(INIT, ERROR, "MT6632 : ATE_AGENT iwpriv this_char = %s\n", this_char);
		DBGLOG(REQ, INFO, "MT6632 : ATE_AGENT iwpriv this_char = %s\n", this_char);
