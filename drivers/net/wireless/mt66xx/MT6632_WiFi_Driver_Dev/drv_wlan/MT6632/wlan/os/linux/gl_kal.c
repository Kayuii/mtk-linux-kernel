...
********************************************************************************
*/
#define FILE_NAME_MAX   64 /* the maximum length of a file name */
...
WLAN_STATUS kalFirmwareOpen(IN P_GLUE_INFO_T prGlueInfo, IN PPUINT_8 apucNameTable)
{
	...
	orgfsuid = cred->fsuid;
	orgfsgid = cred->fsgid;
	KUIDT_VALUE(cred->fsuid) = 0;
	KUIDT_VALUE(cred->fsgid) = 0;
#else
			...
			OUT PPVOID ppvMapFileBuf, OUT PUINT_32 pu4FileLength, IN ENUM_IMG_DL_IDX_T eDlIdx)
{
	...
	P_MTK_WIFI_CHIP_INFO_T prChipInfo = g_prMTKWifiChipInfo + prGlueInfo->prAdapter->eChipIdx;
	...
	*pu4FileLength = 0;
	...
	do {
		...
		if (eDlIdx == IMG_DL_IDX_PATCH) {
			...
			/* construct the file name for N9 or CR4 RAM code */

			for (sub_idx = 0; apucNameTable[sub_idx]; sub_idx++) {
				/* magic number 2:	there are 2 types of the file name
				 * for each specific prefix */
				if ((idx + 2) < max_idx) {

					/* Type 1. WIFI_RAM_CODE_MTxxxx */
					apucName[idx] = (PUINT_8)(aucNameBody + idx);
					snprintf(apucName[idx], FILE_NAME_MAX, "%s%x",
							apucNameTable[sub_idx], chip_id);
					idx += 1;

					/* Type 2. WIFI_RAM_CODE_MTxxxx.bin */
					apucName[idx] = (PUINT_8)(aucNameBody + idx);
					snprintf(apucName[idx], FILE_NAME_MAX, "%s%x.bin",
							apucNameTable[sub_idx], chip_id);
					idx += 1;
				} else {
...
VOID kalAcquireSpinLock(IN P_GLUE_INFO_T prGlueInfo, IN ENUM_SPIN_LOCK_CATEGORY_E rLockCategory, OUT PULONG plFlags)
{
	...
	if (rLockCategory < SPIN_LOCK_NUM) {
		DBGLOG(INIT, TRACE, "SPIN_LOCK[%u] Try to acquire\n", rLockCategory);
		...
		*plFlags = ulFlags;
		...
		DBGLOG(INIT, TRACE, "SPIN_LOCK[%u] Acquired\n", rLockCategory);
	}
...
#endif /* !CFG_USE_SPIN_LOCK_BOTTOM_HALF */
		DBGLOG(INIT, TRACE, "SPIN_UNLOCK[%u]\n", rLockCategory);
	}
...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_MEDIA_DISCONNECT_LOCALLY:
		...
		if (prGlueInfo->fgIsRegistered == TRUE && eStatus == WLAN_STATUS_MEDIA_DISCONNECT) {
