...
int mtk_p2p_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
...
#if 0 /* Reserve the flow unregister in CFG ops */
	/* TODO: */
	P_GLUE_INFO_T prGlueInfo = (P_GLUE_INFO_T) NULL;
	UINT_8 ucRoleIdx;
	P_ADAPTER_T prAdapter;
	...
	DBGLOG(P2P, TRACE, "mtk_p2p_cfg80211_del_iface\n");
	...
	for (ucRoleIdx = 1; ucRoleIdx < BSS_P2P_NUM; ucRoleIdx++) {
		...
		free_netdev(prGlueInfo->prP2PInfo[ucRoleIdx]->aprRoleHandler);
		prGlueInfo->prP2PInfo[ucRoleIdx]->aprRoleHandler = NULL;
	}
#endif

	prP2pRoleFsmInfo = prAdapter->rWifiVar.aprP2pRoleFsmInfo[0];
	/*prP2pInfo->prWdev = gprP2pWdev[0];*/
	/* 13. bind netdev pointer to netdev index */
	wlanBindBssIdxToNetInterface(prGlueInfo, prP2pRoleFsmInfo->ucBssIndex,
	...
	p2pRoleFsmUninit(prGlueInfo->prAdapter, 0);

	rtnl_lock();
	/* Here are functions which need rtnl_lock */
	unregister_netdev(UnregRoleHander);
	free_netdev(UnregRoleHander);
	rtnl_unlock();

	prNetDevPriv = (P_NETDEV_PRIVATE_GLUE_INFO) netdev_priv(prGlueInfo->prP2PInfo[0]->prDevHandler);
	...
	wlanBindBssIdxToNetInterface(prGlueInfo, prNetDevPriv->ucBssIdx,
					 (PVOID) prGlueInfo->prP2PInfo[0]->aprRoleHandler);


	/*kfree(gprP2pRoleWdev[0]);*/
	gprP2pRoleWdev[0] = NULL;
#endif
		return 0;
...
#if 0
		P_BSS_INFO_T BssInfoPtr;

		BssInfoPtr = prGlueInfo->prAdapter->aprBssInfo[rKey.ucBssIdx];
		ASSERT(IS_BSS_ACTIVE(BssInfoPtr));
		if (BssInfoPtr->eCurrentOPMode == OP_MODE_ACCESS_POINT)
			rKey.u4KeyIndex |= BIT(31);	/* Tx */
#else
