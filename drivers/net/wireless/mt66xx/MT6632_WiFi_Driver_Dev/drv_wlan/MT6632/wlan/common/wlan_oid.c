	...
	*pu4SetInfoLen = u4SetBufferLen;
...

	{
		...
		if (1) {
			...
			/* Todo:: check and assert if not ap mode and the differnet between AP and STA */
	...
	*pu4SetInfoLen = sizeof(PARAM_REMOVE_KEY_T);
	...
	if (!prStaRec) {
		...
		if ((IS_BSS_AIS(prBssInfo) &&
		    ...
		    (IS_BSS_P2P(prBssInfo) && 1 /* Todo:: AP wep */)) {
			fgRemoveWepKey = TRUE;
		}
#else
		...
		if (fgRemoveWepKey) {
			...
			/* kalMemZero(prBssInfo->wepkey[u4KeyIndex], 16); */
			prBssInfo->wepkeyUsed[u4KeyIndex] = FALSE;
			/* remove the wlan index here ? */
	...
	if (prRemovedKey->u4KeyIndex < 4) {	/* BIP */
			 ...
			 OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_EFUSE_BUFFER_MODE_T);
	...
	kalMemCopy(rCmdSetEfuseBufModeInfo.aBinContent, prSetEfuseBufModeInfo->aBinContent,
			sizeof(UINT_8) * EFUSE_CONTENT_SIZE);
	...
	rWlanStatus = wlanSendSetQueryExtCmd(prAdapter,
						...
						(PUINT_8) (&rCmdSetEfuseBufModeInfo), pvSetBuffer, u4SetBufferLen);
#else
	rCmdSetEfuseBufModeInfo.ucSourceMode = prSetEfuseBufModeInfo->ucSourceMode;
	rCmdSetEfuseBufModeInfo.ucCount = prSetEfuseBufModeInfo->ucCount;
	kalMemCopy(rCmdSetEfuseBufModeInfo.aBinContent, prSetEfuseBufModeInfo->aBinContent,
...
#if CFG_SUPPORT_QA_TOOL
		rCmdAccessReg.ucAntIdx = prMcrRdInfo->ucMcrAntIdx;
#endif /* CFG_SUPPORT_QA_TOOL */
		   ...
		   IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_MCR_RW_STRUCT_T);
	...
	if (prMcrWrInfo->u4McrOffset & 0xFFFF0000) {
		...
		rCmdAccessReg.u4Data = prMcrWrInfo->u4McrData;
#if CFG_SUPPORT_QA_TOOL
		rCmdAccessReg.ucAntIdx = prMcrWrInfo->ucMcrAntIdx;
#endif /* CFG_SUPPORT_QA_TOOL */
		       ...
		       IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
	...
	case 0xBABA:
		...
		case 0x04:
			halDumpHifStatus(prAdapter->prGlueInfo);
			break;
