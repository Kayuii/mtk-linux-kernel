...
P_QUE_T qmDetermineStaTxQueue(IN P_ADAPTER_T prAdapter, IN P_MSDU_INFO_T prMsduInfo, OUT PUINT_8 pucTC)
{
	...
	if (prStaRec->fgIsTxAllowed) {
		...
		&& kalIsPairwiseEapolPacket(prMsduInfo->prPacket)) {
	...
	/* 4 <2> Adjust TcqStatus according to the updated prQM->au4CurrentTcResource */
		...
		for (i = 0; i < QM_ACTIVE_TC_NUM; i++) {
			...
			if (ai4ExtraQuota[i] > 0) {	/* The resource shall be reallocated to other TCs */
				...
				prTcqAdjust->ai4Variation[i] = (INT_8) (-ai4ExtraQuota[i]);
			}
		}
...
VOID qmProcessPktWithReordering(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, OUT P_QUE_T prReturnedQue)
{
	...
	switch (fgIsAmsduSubframe) {
	case RX_PAYLOAD_FORMAT_FIRST_SUB_AMSDU:
		prReorderQueParm->prMpduSwRfb = prSwRfb;
	...
	case RX_PAYLOAD_FORMAT_MIDDLE_SUB_AMSDU:
		...
		prReorderQueParm->u4SeqNo = u4SeqNo;
		QUEUE_INSERT_TAIL(&prMpduSwRfb->rAmsduQue, (P_QUE_ENTRY_T) prSwRfb);
		return;
	...
	case RX_PAYLOAD_FORMAT_LAST_SUB_AMSDU:
		...
		prReorderQueParm->prMpduSwRfb = NULL;
		break;
...
#if CFG_SUPPORT_RX_AMSDU
		QUEUE_REMOVE_HEAD(&prSwRfb->rAmsduQue, prQueueEntry, P_QUE_ENTRY_T);
		while (prQueueEntry) {
			((P_SW_RFB_T) prQueueEntry)->eDst = RX_PKT_DESTINATION_NULL;
			QUEUE_INSERT_TAIL(prReturnedQue, prQueueEntry);
			QUEUE_REMOVE_HEAD(&prSwRfb->rAmsduQue, prQueueEntry, P_QUE_ENTRY_T);
		}
#endif
		return;
...
#if CFG_SUPPORT_RX_AMSDU
			QUEUE_REMOVE_HEAD(&prReorderedSwRfb->rAmsduQue, prQueueEntry, P_QUE_ENTRY_T);
			while (prQueueEntry) {
				QUEUE_INSERT_TAIL(prReturnedQue, prQueueEntry);
				QUEUE_REMOVE_HEAD(&prReorderedSwRfb->rAmsduQue, prQueueEntry, P_QUE_ENTRY_T);
			}
...
VOID qmDumpQueueStatus(IN P_ADAPTER_T prAdapter)
{
	...
	for (i = TC0_INDEX; i < TC_NUM; i++) {
		DBGLOG(SW4, INFO, "TC%u ResCount: Max[%02u/%03u] Free[%02u/%03u] PreUsed[%03u]\n",
		       ...
		       prTxCtrl->rTc.au4FreePageCount[i], prTxCtrl->rTc.au4PreUsedPageCount[i]);
