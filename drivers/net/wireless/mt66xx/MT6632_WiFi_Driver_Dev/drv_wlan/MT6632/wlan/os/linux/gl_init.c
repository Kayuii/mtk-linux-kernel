...
#warning "CONFIG_ANDROID is undefined."
#endif
...
static INT_32 wlanProbe(PVOID pvData)
{
	...
	do {
		...
		if (wlanAdapterStart(prAdapter, prRegInfo) != WLAN_STATUS_SUCCESS)
			...
			DBGLOG(INIT, INFO, "support CFG_SUPPORT_SLT_DUT_KO. return %s\n", __FUNCTION__);
			i4Status = 0;
			return i4Status;			
		}
#endif
		...
		if (prAdapter->rWifiVar.ucEfuseBufferModeCal == TRUE) {
			...
			prChipInfo = g_prMTKWifiChipInfo + prAdapter->eChipIdx;
			chip_id = prChipInfo->chip_id;

			/* 1 <1> Load bin file*/
			pucConfigBuf = (PUINT_8) kalMemAlloc(1200, VIR_MEM_TYPE);
			kalMemZero(pucConfigBuf, 1200);

			snprintf(aucEeprom, 20, "%s%x.bin",
					apucEepromName[0], chip_id);

			kalSprintf(aucEepromName, "%s%s", apucFwPath[1], aucEeprom);


			if (pucConfigBuf) {

				if (kalReadToFile(aucEepromName,
					pucConfigBuf, 1200, &u4ConfigReadLen) == 0) {
					/* ToDo:: Nothing */
					DBGLOG(INIT, INFO, "open file done\n");
				} else {
			...
			/* 1 <2> Send CMD with bin file content */

			prGlueInfo = prAdapter->prGlueInfo;
			for (u4LoopCount = 0 ; u4LoopCount < EFUSE_CONTENT_SIZE ; u4LoopCount++) {
			...
			kalMemFree(pucConfigBuf, VIR_MEM_TYPE, 1200);
		}
#endif
#endif
