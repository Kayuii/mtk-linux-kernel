...
BOOLEAN glRegisterP2P(P_GLUE_INFO_T prGlueInfo, const char *prDevName, const char *prDevName2, UINT_8 ucApMode)
{
	...
	/* *((P_GLUE_INFO_T *) netdev_priv(prGlueInfo->prP2PInfo->prDevHandler)) = prGlueInfo; */
		...
		rMacAddr[0] ^= 0x2 + i*0x2;	/* change to local administrated address */
				    ...
				    IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	case P2P_PROVISIONING_SUCCESS:
		...
		prGlueInfo->prP2PInfo[0]->u4CipherPairwise = IW_AUTH_CIPHER_CCMP;
	...
	if (prData->flags == P2P_STOP_DISCOVERY) {
		...
		if (prReqDeviceType->probe_rsp_len > 0) {
			if (prReqDeviceType->probe_rsp_len <= MAX_IE_LENGTH) {
				if (copy_from_user
				    (prGlueInfo->prP2PInfo[0]->aucWSCIE[2],
				     prReqDeviceType->probe_rsp_ie, prReqDeviceType->probe_rsp_len)) {
					return -EFAULT;
				}
				prGlueInfo->prP2PInfo[0]->u2WSCIELen[2] = prReqDeviceType->probe_rsp_len;
			} else {
				return -E2BIG;
			}
		}
		...
		if (prReqDeviceType->probe_req_len > 0) {
			if (prReqDeviceType->probe_req_len <= MAX_IE_LENGTH) {
				if (copy_from_user
				    (prGlueInfo->prP2PInfo[0]->aucWSCIE[1],
				     prReqDeviceType->probe_req_ie, prReqDeviceType->probe_req_len)) {
					return -EFAULT;
				}
				prGlueInfo->prP2PInfo[0]->u2WSCIELen[1] = prReqDeviceType->probe_req_len;
			} else {
				return -E2BIG;
			}
		}
		      ...
		      IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	case IW_AUTH_CIPHER_PAIRWISE:
		prGlueInfo->prP2PInfo[0]->u4CipherPairwise = prAuth->value;
		break;
			       ...
			       IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	P_GLUE_INFO_T prGlueInfo = NULL;
	P_ADAPTER_T prAdapter = NULL;
	P_P2P_INFO_T prP2PInfo[0] = (P_P2P_INFO_T) NULL;
	...
	prP2PInfo[0] = prAdapter->prP2PInfo[0];

	for (i = 0; i < prP2PInfo[0]->u4DeviceNum; i++) {
		prTargetResult = &prP2PInfo[0]->arP2pDiscoverResult[i];
		    ...
		    IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	if (prHostapdParam->len > 0) {
		if (prHostapdParam->len <= MAX_WSC_IE_LENGTH) {
			if (copy_from_user
			    (prGlueInfo->prP2PInfo[0]->aucWSCIE[0], prHostapdParam->data, prHostapdParam->len)) {
				return -EFAULT;
			}
			if (copy_from_user
			    (prGlueInfo->prP2PInfo[0]->aucWSCIE[2], prHostapdParam->data, prHostapdParam->len)) {
				return -EFAULT;
			}
		} else {
			return -E2BIG;
	...
	prGlueInfo->prP2PInfo[0]->u2WSCIELen[0] = prHostapdParam->len;
	prGlueInfo->prP2PInfo[0]->u2WSCIELen[2] = prHostapdParam->len;
			    ...
			    IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	if (prPasswordReady->probe_req_len > 0) {
		if (prPasswordReady->probe_req_len <= MAX_WSC_IE_LENGTH) {
			if (copy_from_user
			    (prGlueInfo->prP2PInfo[0]->aucWSCIE[1], prPasswordReady->probe_req_ie,
	...
	prGlueInfo->prP2PInfo[0]->u2WSCIELen[1] = prPasswordReady->probe_req_len;

	/* retrieve IE for Probe Response */
	if (prPasswordReady->probe_rsp_len > 0) {
		if (prPasswordReady->probe_rsp_len <= MAX_WSC_IE_LENGTH) {
			if (copy_from_user
			    (prGlueInfo->prP2PInfo[0]->aucWSCIE[2], prPasswordReady->probe_rsp_ie,
	...
	prGlueInfo->prP2PInfo[0]->u2WSCIELen[2] = prPasswordReady->probe_rsp_len;
			      ...
			      IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	kalMemCopy(prDeviceReq->name, prGlueInfo->prP2PInfo[0]->aucConnReqDevName,
		   prGlueInfo->prP2PInfo[0]->u4ConnReqNameLength);
	prDeviceReq->name_len = prGlueInfo->prP2PInfo[0]->u4ConnReqNameLength;
	prDeviceReq->name[prDeviceReq->name_len] = '\0';
	COPY_MAC_ADDR(prDeviceReq->device_addr, prGlueInfo->prP2PInfo[0]->rConnReqPeerAddr);
	prDeviceReq->device_type = prGlueInfo->prP2PInfo[0]->ucConnReqDevType;
	prDeviceReq->config_method = prGlueInfo->prP2PInfo[0]->i4ConnReqConfigMethod;
	prDeviceReq->active_config_method = prGlueInfo->prP2PInfo[0]->i4ConnReqActiveConfigMethod;
				 ...
				 IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	/* copy to upper-layer supplied buffer */
	kalMemCopy(prInvIndicate->dev_name, prGlueInfo->prP2PInfo[0]->aucConnReqDevName,
		   prGlueInfo->prP2PInfo[0]->u4ConnReqNameLength);
	kalMemCopy(prInvIndicate->group_bssid, prGlueInfo->prP2PInfo[0]->rConnReqGroupAddr, MAC_ADDR_LEN);
	prInvIndicate->name_len = prGlueInfo->prP2PInfo[0]->u4ConnReqNameLength;
	prInvIndicate->dev_name[prInvIndicate->name_len] = '\0';
	COPY_MAC_ADDR(prInvIndicate->dev_addr, prGlueInfo->prP2PInfo[0]->rConnReqPeerAddr);
	prInvIndicate->config_method = prGlueInfo->prP2PInfo[0]->i4ConnReqConfigMethod;
	prInvIndicate->operating_channel = prGlueInfo->prP2PInfo[0]->ucOperatingChnl;
	prInvIndicate->invitation_type = prGlueInfo->prP2PInfo[0]->ucInvitationType;
			       ...
			       IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	/* copy to upper-layer supplied buffer */
	prInvStatus->status_code = prGlueInfo->prP2PInfo[0]->u4InvStatus;
		     ...
		     IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	DBGLOG(INIT, WARN, "set parameter, u4SubCmd=%d idx=%d value=%lu\n", (INT_16) u4SubCmd, (INT_16) index, value);
			...
			IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	kalMemZero(&prGlueInfo->prP2PInfo[0]->aucOidBuf[0], sizeof(prGlueInfo->prP2PInfo[0]->aucOidBuf));

	switch (u4SubCmd) {
	case PRIV_CMD_OID:
		if (copy_from_user(&(prGlueInfo->prP2PInfo[0]->aucOidBuf[0]), wrqu->data.pointer, wrqu->data.length)) {
			status = -EFAULT;
			break;
		}

		if (!kalMemCmp(&(prGlueInfo->prP2PInfo[0]->aucOidBuf[0]), extra, wrqu->data.length))
			DBGLOG(INIT, WARN, "extra buffer is valid\n");
		else
			DBGLOG(INIT, WARN, "extra 0x%p\n", extra);

		prP2PReq = (P_IW_P2P_TRANSPORT_STRUCT) (&(prGlueInfo->prP2PInfo[0]->aucOidBuf[0]));
	...
	case PRIV_SEC_CHECK_OID:
		...
		if (copy_from_user(&(prGlueInfo->prP2PInfo[0]->aucSecCheck[0]),
			wrqu->data.pointer, wrqu->data.length)) {
			status = -EFAULT;
			break;
		}

		if (!kalMemCmp(&(prGlueInfo->prP2PInfo[0]->aucSecCheck[0]), extra, wrqu->data.length))
			DBGLOG(INIT, WARN, "extra buffer is valid\n");
		else
			DBGLOG(INIT, WARN, "extra 0x%p\n", extra);
		prP2PReq = (P_IW_P2P_TRANSPORT_STRUCT) (&(prGlueInfo->prP2PInfo[0]->aucSecCheck[0]));
			...
			IN struct iw_request_info *info, IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	switch (u4SubCmd) {
	case PRIV_CMD_OID:
		if (copy_from_user(&(prGlueInfo->prP2PInfo[0]->aucOidBuf[0]),
		...
		case P2P_CMD_ID_INVITATION_INDICATE:
			{
				...
				if (copy_to_user(wrqu->data.pointer,
						 &(prGlueInfo->prP2PInfo[0]->aucOidBuf[0]),
