...
#if CFG_USB_TX_AGG
#define USB_REQ_TX_DATA_CNT             (32)	/* must be >= 2 */
#else
 ...
 */
#define USB_TX_DATA_BUFF_SIZE			32768
#endif
#define USB_RX_EVENT_BUF_SIZE           (USB_RX_AGGREGTAION_LIMIT * 1000)
#define USB_RX_DATA_BUF_SIZE            (USB_RX_AGGREGTAION_LIMIT * 1000)
...
typedef struct _GL_HIF_USB_FUNC_T {
	struct urb *pUrb;
	dma_addr_t pUrbBuf;
	UINT_32 u4BulkInEndPointAddr;
	struct _GL_HIF_INFO_T *prHifInfo;
} GL_HIF_USB_FUNC_T, *P_GL_HIF_USB_FUNC_T;
#endif
...
#if 0
	GL_HIF_USB_FUNC_T arUsbFunc[IDX_RX_PORT_MAX];
#endif

	spinlock_t rQLock;
	PVOID prTxCmdReqHead;
	PVOID arTxDataReqHead[TC_NUM];
	PVOID prRxEventReqHead;
	PVOID prRxDataReqHead;
	struct list_head rTxCmdFreeQ;
	struct list_head rTxCmdSendingQ;
#if CFG_USB_TX_AGG
	struct list_head rTxDataFreeQ[TC_NUM];
	struct list_head rTxDataSendingQ[TC_NUM];
#else
...
#if CFG_USB_TX_AGG
	BUF_CTRL_T rTxDataBufCtrl[TC_NUM][USB_REQ_TX_DATA_CNT];
#else
