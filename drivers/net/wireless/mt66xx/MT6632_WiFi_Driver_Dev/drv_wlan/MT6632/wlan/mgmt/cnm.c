...
VOID cnmUpdateDbdcSetting(P_ADAPTER_T prAdapter)
{
	BOOLEAN fgDbdcEn = FALSE;
	...
	prCmdBody->ucUpdateSettingNextChReq = FALSE;
...
VOID cnmGetDbdcCapability(P_ADAPTER_T prAdapter, UINT_8 ucBssIndex,
	ENUM_BAND_T eRfBand, UINT_8 ucPrimaryChannel, UINT_8 ucNss,
	P_CNM_DBDC_CAP_T prDbdcCap)
{
	if (!prDbdcCap)
		return;
	...
	case DBDC_MODE_STATIC:
		...
		if (eRfBand == BAND_5G) {
			...
			prDbdcCap->ucWmmSetIndex = 1;
		}
		break;

	case DBDC_MODE_DYNAMIC:
		/* Dynamic DBDC mode, TODO! */
		break;
