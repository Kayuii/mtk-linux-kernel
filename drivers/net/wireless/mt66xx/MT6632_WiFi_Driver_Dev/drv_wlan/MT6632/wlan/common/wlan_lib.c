...
struct _MTK_WIFI_CHIP_INFO_T mtk_wifi_chip_info[] = {
	{0x6632, WIFI_CFG_SW_SYNC0, WIFI_CFG_SYNC0_RDY_OFFSET},
	{0x7666, WIFI_CFG_SW_SYNC0, WIFI_CFG_SYNC0_RDY_OFFSET},
	{0x7668, WIFI_RGU_SW_SYNC0, WIFI_RGU_SYNC0_RDY_OFFSET}
};

P_MTK_WIFI_CHIP_INFO_T g_prMTKWifiChipInfo = mtk_wifi_chip_info;
...
WLAN_STATUS wlanAdapterStart(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	do {
...
#if CFG_SUPPORT_SLT_DUT_KO 
		if (u4Status == WLAN_STATUS_SUCCESS) {
			DBGLOG(INIT, INFO, "support CFG_SUPPORT_SLT_DUT_KO. break %s\n", __FUNCTION__);
			//return u4Status;
			break;
		}
#endif
		...
		cnmUpdateDbdcSetting(prAdapter);

	} while (FALSE);

	if (u4Status == WLAN_STATUS_SUCCESS) {

#if defined(_HIF_USB) && 0
#else
	...
	do {
		...
		while (prQueueEntry) {
			...
			if (prCmdInfo->fgIsOid) {
				...
				DBGLOG(OID, INFO, "Cancel pending USB CMD ID[0x%02X] SEQ[%u] buf[0x%p]\n",
				       prCmdInfo->ucCID, prCmdInfo->ucCmdSeqNum, prCmdInfo->pucInfoBuffer);
...
VOID wlanSetChipEcoInfo(IN P_ADAPTER_T prAdapter)
{
	...
	if (wlanAccessRegister(prAdapter, TOP_HVR, &hw_version, 0, 0) != WLAN_STATUS_SUCCESS) {
		...
		nicSetChipFactoryVer((UINT_8)((GET_HW_VER(hw_version) >> 16) & 0xF));
...
VOID wlanSetNicResourceParameters(IN P_ADAPTER_T prAdapter)
{
	...
	prWifiVar->au4TcPageCount[TC4_INDEX] = prAdapter->nicTxReousrce.u4McuTotalResource;	 /* MCU */
	...
	prWifiVar->au4TcPageCount[TC3_INDEX] += prAdapter->nicTxReousrce.u4LmacTotalResource%(TC_NUM - 1);
...
VOID wlanInitFeatureOption(IN P_ADAPTER_T prAdapter)
{
	...
	prWifiVar->ucStaVhtBfee = (UINT_8) wlanCfgGetUint32(prAdapter, "StaVHTBfee", FEATURE_DISABLED);
	 ...
	 * 2: dynamic DBDC
	 */
	prWifiVar->ucDbdcMode = (UINT_8) wlanCfgGetUint32(prAdapter, "DbdcMode", DBDC_MODE_DISABLED);
#if (CFG_EFUSE_BUFFER_MODE_DELAY_CAL == 1)
