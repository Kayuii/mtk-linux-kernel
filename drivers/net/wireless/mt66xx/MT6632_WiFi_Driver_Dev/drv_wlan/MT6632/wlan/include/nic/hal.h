...
#define HAL_WRITE_TX_DATA(_prAdapter, _prMsduInfo, _ucIsLastMsduInfo) \
{ \
	if (_prAdapter->rAcpiState == ACPI_STATE_D3) { \
		ASSERT(0); \
	} \
	kalDevWriteData(_prAdapter->prGlueInfo, _prMsduInfo, _ucIsLastMsduInfo); \
}
...
do { \
	...
	UINT_32 u4Value; \
	if (_prAdapter->eChipIdx >= MTK_WIFI_CHIP_MAX) \
		ASSERT(0); \
	*_pfgResult = FALSE; \
	prChipInfo = g_prMTKWifiChipInfo + _prAdapter->eChipIdx; \
	HAL_MCR_RD(_prAdapter, WIFI_CFG_SW_SYNC0, &u4Value); \
...
#define HAL_WIFI_FUNC_GET_STATUS(_prAdapter, _u4Result) \
	nicGetMailbox(_prAdapter, 0, &_u4Result)
