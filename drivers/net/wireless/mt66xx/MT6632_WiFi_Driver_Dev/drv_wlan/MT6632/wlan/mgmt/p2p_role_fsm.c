...
UINT_8 p2pRoleFsmInit(IN P_ADAPTER_T prAdapter, IN UINT_8 ucRoleIdx)
{
	...
	do {
		...
		prP2pBssInfo->aucOwnMacAddr[0] ^= 0x2 + ucRoleIdx*0x2;	/* change to local administrated address */
...
VOID p2pRoleFsmRunEventStartAP(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		/* Update DBDC setting in BSS INFO */
		...
		DBGLOG(P2P, INFO,
		   "p2pRoleFsmRunEventStartAP: start AP at band %u, NSS=%u.\n",
		   rDbdcCap.ucDbdcBandIndex,
		   rDbdcCap.ucNss);
...
VOID p2pRoleFsmRunEventConnectionRequest(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		if (prJoinInfo->prTargetBssDesc == NULL) {
			...
			DBGLOG(P2P, INFO,
			   "p2pRoleFsmRunEventConnectionRequest: start GC at band %u, NSS=%u.\n",
			   rDbdcCap.ucDbdcBandIndex,
			   rDbdcCap.ucNss);
		...
		case P2P_ROLE_STATE_SCAN:
			...
			if (prConnReqInfo->eConnRequest == P2P_CONNECTION_TYPE_GC) {
				...
				if ((prP2pJoinInfo->prTargetBssDesc) == NULL) {
					...
					DBGLOG(P2P, INFO,
						"p2pRoleFsmRunEventScanDone: start GC at band %u, NSS=%u.\n",
						rDbdcCap.ucDbdcBandIndex,
						rDbdcCap.ucNss);
