...
BOOLEAN nicTxCalculateResource(IN P_ADAPTER_T prAdapter, IN UINT_16 * au2TxRlsCnt, OUT UINT_16 * au2FreeTcResource)
{
	...
	for (ucTcIdx = TC0_INDEX; ucTcIdx < TC_NUM; ucTcIdx++) {
		...
		au4TxDoneCnt[ucTcIdx] += au2TxRlsCnt[arTcResourceControl[ucTcIdx].ucHifTxQIndex];
	...
	DBGLOG(TX, TRACE, "Tx Done INT result, AC5/6[%u:%u] BMC/BCN[%u:%u] AC10~14[%u:%u:%u:%u:%u]\n",
	       ...
	       au2TxRlsCnt[HIF_TX_AC12_INDEX], au2TxRlsCnt[HIF_TX_AC13_INDEX], au2TxRlsCnt[HIF_TX_AC14_INDEX]);
#endif
...
WLAN_STATUS nicTxMsduQueue(IN P_ADAPTER_T prAdapter, UINT_8 ucPortIdx, P_QUE_T prQue)
{
	...
	while (QUEUE_IS_NOT_EMPTY(prQue)) {
		...
		if (!halTxIsDataBufEnough(prAdapter, prMsduInfo->ucTC, prMsduInfo->u2FrameLength + NIC_TX_HEAD_ROOM)) {
		...
		HAL_WRITE_TX_DATA(prAdapter, prMsduInfo, QUEUE_IS_EMPTY(prQue));
	}
