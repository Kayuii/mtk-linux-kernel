...
static const struct usb_device_id mtk_usb_ids[] = {
	...
	{USB_DEVICE_AND_INTERFACE_INFO(0x0E8D, 0x6632, 0xff, 0xff, 0xff), .driver_info = MT_MAC_BASE},
	{USB_DEVICE_AND_INTERFACE_INFO(0x0E8D, 0x7666, 0xff, 0xff, 0xff), .driver_info = MT_MAC_BASE},
	{USB_DEVICE_AND_INTERFACE_INFO(0x0E8D, 0x7668, 0xff, 0xff, 0xff), .driver_info = MT_MAC_BASE},
	{ /* end: all zeroes */ },
};
...
static int mtk_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (pfWlanProbe((PVOID) intf) != WLAN_STATUS_SUCCESS) {
...
{
	/* printk(KERN_INFO DRV_NAME "mtk_usb_suspend()\n"); */
...
{
	/* printk(KERN_INFO DRV_NAME "mtk_usb_resume()\n"); */
...
{
	/* printk(KERN_INFO DRV_NAME "mtk_usb_reset_resume()\n"); */
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This function is to free HIF USB FUNC (URB)
*
* \param[in] P_GL_HIF_INFO_T Pointer to HIF info structure
*
* \return (none)
*/
/*----------------------------------------------------------------------------*/
static void freeHifUSBFunc(P_GL_HIF_INFO_T prHif)
{
	ASSERT(prHif);
	UINT_32 i = 0;

	for (i = 0; i < IDX_RX_PORT_MAX; i++) {
		if (prHif->arUsbFunc[i].pUrbBuf != NULL) {
			kfree(prHif->arUsbFunc[i].pUrbBuf);
			prHif->arUsbFunc[i].pUrbBuf = NULL;
		}
		if (prHif->arUsbFunc[i].pUrb != NULL) {
			usb_kill_urb(prHif->arUsbFunc[i].pUrb);
			usb_free_urb(prHif->arUsbFunc[i].pUrb);
			prHif->arUsbFunc[i].pUrb = NULL;
		}
	}
}
...
{
	P_GL_HIF_INFO_T prHif = &prGlueInfo->rHifInfo;
	UINT_32 i = 0;

	prHif->intf = ulCookie;
	prHif->udev = interface_to_usbdev(prHif->intf);

	prHif->prGlueInfo = prGlueInfo;
	usb_set_intfdata(prHif->intf, prGlueInfo);

	SET_NETDEV_DEV(prGlueInfo->prDevHandler, &prHif->udev->dev);

	mutex_init(&prHif->vendor_req_sem);

	/* init URB */
	for (i = 0; i < IDX_RX_PORT_MAX; i++) {
		prHif->arUsbFunc[i].u4BulkInEndPointAddr =
		    ((i == IDX_RX_PORT_EVENT) ? USB_EVENT_EP_IN : USB_DATA_EP_IN);
		prHif->arUsbFunc[i].prHifInfo = prHif;
		prHif->arUsbFunc[i].pUrb = usb_alloc_urb(0, GFP_ATOMIC);
		if (prHif->arUsbFunc[i].pUrb == NULL) {
			DBGLOG(HAL, ERROR, ("usb_alloc_urb() reports error\n"));
			goto error;
		}
		prHif->arUsbFunc[i].pUrbBuf = kmalloc(CFG_RX_MAX_PKT_SIZE, GFP_ATOMIC | GFP_DMA);
		if (prHif->arUsbFunc[i].pUrbBuf == NULL) {
...
BOOL kalDevRegRead(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4Register, OUT PUINT_32 pu4Value)
{
	...
	do {
		mtk_usb_vendor_request(prGlueInfo, 0, DEVICE_VENDOR_REQUEST_IN, VND_REQ_REG_READ,
...
BOOL kalDevRegWrite(IN P_GLUE_INFO_T prGlueInfo, IN UINT_32 u4Register, IN UINT_32 u4Value)
{
	...
	do {
		mtk_usb_vendor_request(prGlueInfo, 0, DEVICE_VENDOR_REQUEST_OUT, VND_REQ_REG_WRITE,
