...
static const struct sdio_device_id mtk_sdio_ids[] = {
	{SDIO_DEVICE(0x037a, 0x6602)},	/* Not an SDIO standard class device */
	{SDIO_DEVICE(0x037a, 0x7608)},  /* Not an SDIO standard class device */
	{ /* end: all zeroes */ },
};
...
BOOL kalDevWriteData(IN P_GLUE_INFO_T prGlueInfo, IN P_MSDU_INFO_T prMsduInfo, IN UINT_8 ucIsLastMsduInfo)
{
	...
	if (prTxCtrl->u4WrIdx + ALIGN_4(u4Length) > prAdapter->u4CoalescingBufCachedSize) {
		DBGLOG(HAL, ERROR, "Data TX buffer underflow!\n");
		return FALSE;
	}
	...
	if (!prMsduInfo->pfTxDoneHandler) {
		...
		QUEUE_INSERT_TAIL(&prHifInfo->rFreeQueue, (P_QUE_ENTRY_T) prMsduInfo);
	}
#else
	if (!prMsduInfo->pfTxDoneHandler) {
		/* Free MSDU_INFO */
		if (prMsduInfo->eSrc == TX_PACKET_OS) {
			...
			kalSendComplete(prAdapter->prGlueInfo, prMsduInfo->prPacket, WLAN_STATUS_SUCCESS);
	...
	if (ucIsLastMsduInfo == 1) {
		if ((prAdapter->u4CoalescingBufCachedSize - prTxCtrl->u4WrIdx) >= sizeof(UINT_32)) {
		...
		nicTxReturnMsduInfo(prAdapter, (P_MSDU_INFO_T) QUEUE_GET_HEAD(&prHifInfo->rFreeQueue));

		QUEUE_INITIALIZE(&prHifInfo->rFreeQueue);
...
/*	WLAN_STATUS u4Status = WLAN_STATUS_SUCCESS; */
	...
	if ((prAdapter->u4CoalescingBufCachedSize - u2OverallBufferLength) >= sizeof(UINT_32)) {
		/* fill with single dword of zero as TX-aggregation termination */
